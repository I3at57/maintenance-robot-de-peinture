...
L'optimisation s'effectuera en deux temps. On cherchera d’abord à construire une
...
En effet, le calcul de $\frac{\mathbb{E}[CC_r]$ et de $\mathbb{E}[T_r]$ doit se
...
```{r}
afficher_variation_cut(seq(110,120,0.1), parametres, p = 10000)
```
...
entre $T = 110$ et $T = 120$, avec un coût unitaire moyen proche de $3€/h$.
...
### Optimisation de la politique de maintenance pour le composant D

Il serait possible d'automatiser le processus précédent pour l'appliquer à tous
les composants du robot (voir Annexe 4).

Cependant, nous utilisons cette politique de maintenance uniquement pour les 
composants de la classe A, nous devons donc identifier la période $T^*_D$.
...
```{r}
afficher_variation_cut(seq(50,500,10), parametres, p = 10000)
```

```{r}
afficher_variation_cut(seq(100,200,1), parametres, p = 5000)
```
```{r}
afficher_variation_cut(seq(100,160,1), parametres, p = 10000)
```

Dans ce cas, le minimum est moins évident à trouver visuellement. Il semble se
situer entre 120h et 140h pour un coût annuel de $4.55€.h^{-1}$.

### Conclusion

Nous avons donc touvé une politique de maintenance adaptée pour les composants
E et D de la classe A, prioritaire.

|Composant|T optimal|Coût moyen estimé par heures|
|E|$110h \sim 120h$|$3€/h$|
|D|$120h \sim 140h$|$4.55€/h$|

On peut facilement envisager une politique commune, visant à effectuer toutes
les maintenances préventives de tous les composants de la classe A.
