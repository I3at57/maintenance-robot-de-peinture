...
A partir de la mise en service du robot (soit la mise en service initiale, soit après le remplacement d'une pièce), on note $T_f$ la durée aléatoire du cycle jusqu'à la
...
Si la maintenance est préventive pour un élément de la classe A, on a :
...
$$
\mathbb{E}[T_r] = (1-R_{\mathbf{W}(\alpha_{\omega},\beta_{\omega})}(T))
(\mathbb{E}[T_f] + \mathbb{E}[D]) + R_{\mathbf{W}(\alpha_{\omega},\beta_{\omega})}
(T+tp)
$$
