...
sont rare et peu impactante sur le temps d'arrêt (10 et 35 minutes), ces deux défaillances
...
Pour les estimations effecutés, plusieurs éléments doivent être prit en compte :
...
L'optimisation s'effectuera en deux temps. On cherchera dabord à determiner le
coût de moyen de maintenance pour les composants de la classe A. Ensuite on
cherchera les périodes de remplacement périodiques optimales pour ce composants.

Nous procèderons à l'optimisation de la politique pour le composant E : "Nez robot".

### Détermination du coût de maintenance moyen par unité de temps

Deux coûts se rapportent à la maintenance d'un composant :

- le coût d'immobilisation du robot $ci = 20€/min$.
- le coût de remplacement, fixe, $cc = 30€$.

Si l'on note $T_r$ la date de remise en service d'un composant après la panne,
le coût moyen par unité de temps de la maintenance sera alors égal à :

$$
\mbox{Coût moyen par unité de temps de la maintenance} =
\frac{E[\mbox{Coût cumulé de la maintenance}]}
{E[T_r]}
$$

#### Détermination de la loi de remise en service

On considère une politique de remplacement basée sut l’âge de l’élément.
A partir de la mise en service du robot (soit la mise en service initiale, soit après le remplacement d'une pièce), on note $T_r$ la durée aléatoire du cycle jusqu'à la
...
- $T_f$ la date alétoire de la prochaine défaillance, qui dépend de la fiabilité du composant.
...
donc conditionner la date $T_r$ au type de remplacement efectué.

La date $T$ de remplacement périodique étant fixé après un temps $T$ de fonctionnement
sans défaillance, le type de remplacement dépend donc de la variable alétoire $T_f$
...
$$
P(\mbox{Remplacement préventif }|\omega) = P(T_f > T|\omega) = R_{\omega}(T)
$$

Avec $R_{\omega}(T)$

Or, pour rappel, $T_f$ pour un composant $\omega$ suit une loi de Weibull de
paramètres $\alpha_{\omega}$ et $\beta_{\omega}$. Donc :

$$
P(\mbox{Remplacement préventif }|\omega) =
R_{\mathbf{W}(\alpha_{\omega},\beta_{\omega})}(T) = 
(1-F_{\mathbf{W}(\alpha_{\omega},\beta_{\omega})}(T))
$$

Puisque $T$ dépend de notre politique de maintenance, et que l'on a déterminé les
parmètres des lois pour tous les composants, alors cette probabilité est fixe.
...
$$
T_r = (1-R_{\mathbf{W}(\alpha_{\omega},\beta_{\omega})}(T))(T_f + D)+
R_{\mathbf{W}(\alpha_{\omega},\beta_{\omega})}(T)(T+tp)
$$
-->
...
On peut donc calculer $\mathbb{E}[T_r]$ pour chacun des composants en fonction des
données disponibles. Les lois de probabilités étant calculés à partir des durée
inter-défaillances, nous ne pourrons pas calculer $\mathbb{E}[T_r]$ pour les
repère dont on ne possède pas ces données (donc les repère H et G).
...
- du coût d'inactivité : $ci = 20€%min$.
...
Dans ce cas si $T_f < T$ alors $CC_r = cc + D \cdot ci = 30 + 20D (€)$
...
<!--
On cherche l'expression du coût de maintenance cumulée, donc sa loi de probabilité.
$CC_r$ est une variable aléatoire continue (car $D$ est une durée en temps continue),
à valeurs dans $\mathbb{R}_+$. On cherche alors
$\mathbb{P}(CC_r \leq k),\forall k \in \mathbb{R}_+$.

On a donc l'expression du coût suivante :
...
On peut alors calculer l'espérance du coût de maintenance total pour $T$ :
...
#### Évaluation de l'espérance du coût moyen de maintenance

On a déjà calculé $\mathbb{E}[CC_r(T)]$ :
...
Ce résultat donne le coût moyen de maintenance à $T_r$ en fonction de $T$. On voit
bien que 
